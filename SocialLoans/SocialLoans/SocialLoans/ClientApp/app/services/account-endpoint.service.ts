...
export class AccountEndpoint extends EndpointFactory {
    ...
    private readonly _myBankaccountsUrl: string = "/api/account/users/me/bankaccounts/"
    ...
    get myBankAccountsUrl() { return this.configurations.baseUrl + this._myBankaccountsUrl; }
    ...
    getMyBankAccountsEndpoint<T>(): Observable<T> {

        return this.http.get<T>(this.myBankAccountsUrl, this.getRequestHeaders())
            .catch(error => {
                return this.handleError(error, () => this.getMyBankAccountsEndpoint());
            })
    }
}